...
function renderRequesterNameOptions() {
  ...
  if (!list) return;
  list.innerHTML = state.requesters.map((name) => `<option value="${escapeHtml(name)}"></option>`).join("");
}

function defaultRequesterName() {
  return state.requesters.includes("王冠楚") ? "王冠楚" : state.requesters[0] || "王冠楚";
}
